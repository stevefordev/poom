...
var $serviceNo = $("input[name=serviceNo]");
...
setLocationFromAddress($detailAddress1.val());
// 맵이 움직일때마다 주소 검색
...
$tag.tagEditor({
  initialTags: $('input[name=tags]').map(function() {
...
function serviceImageLoad(category) {
  ...
  var imageMarkup = serviceImgTemp({
    "imageInfo": imageInfo
  });
  $imageBox.html(imageMarkup);
...
serviceImageLoad($category.val()); // 카테고리에 맞는 이미지 세팅

// 등록할때 선택된 이미지를 선택되도록
var photos = $photo.val().split(',');
$.each(photos, function(key, value) {
  $('input:checkbox[name="images[]"]').map(function() {
...
function imageBoxAddListener() {
  ...
  $imageBox.on('click', 'div.slick-list>div.slick-track>div.slick-slide',
          function() {
            ...
            var vals = $inputCheckboxImges.filter(':checked').map(function() {
...
var tmpSchedules = JSON.parse($scheduleList.val());

// 디비에서 호출한 데이터를 배열에 담고 선택된 날짜 요소에 넣어준다
$.each(tmpSchedules, function() {
  console.log(this);

  if (this.type === 'repeat') { // 반복 일정
    scheduleList.repeatDates[this.serviceDay.slice(0, 3)].times
            .push(this.serviceDay.slice(3, 5));
  } else { // 단일 일정 소스 중복 제거 필요
    var singleDateObject = {
      'date': moment(this.serviceDate).format('YYYY-MM-DD'),
      'times': [this.serviceDateHour]
    };

    var serviceDate = this.serviceDate;

    // 이미 입력된 날짜인지 확인
    var filterSingleDate = scheduleList.singleDates.filter(function(each) {
      return each.date == serviceDate;
    });

    console.log('filterSingleDate', filterSingleDate);
  ...
  _.each(scheduleList.singleDates, function(singleDate) {
    _.each(singleDate.times, function(time) {
      schedules.push({
        "type": "single",
        "serviceStartdate": '',
        "serviceDate": new Date(singleDate.date + ' ' + ('0' + time).slice(-2)
                + ':00:00').getTime()
      })
    })
  })
...
function deleteSchedule(type, date, week, time, $td) {
  ...
  if (date == null || typeof date == 'undefined') {
    serviceDay = week + ("0" + time).slice(-2);
  } else {
    serviceDate = new Date(date + " " + ("0" + time).slice(-2) + ":00:00")
            .getTime();
  }
  console.log(type);
  console.log(date);
...
CKEDITOR.replace('contents', {
  on: {
    instanceReady: function(evt) {
      evt.editor.setData($contents.val());
    }
  }
});
// end
