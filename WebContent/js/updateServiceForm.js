...
$tag.tagEditor({
  initialTags: $('input[name=tags]').map(function(){  return $(this).data('tag') }), // 초기 입력
  maxTags: 5,
  maxLength: 10,
...
$.each(tmpSchedules, function() {
    ...
    if(this.type === 'repeat') { // 반복 일정
      scheduleList.repeatDates[this.serviceDay.slice(0,3)].times.push(this.serviceDay.slice(3,5));
    } else { // 단일 일정 소스 중복 제거 필요
      var singleDateObject = {
              'date': this.serviceDate,
              'times': [new Date(this.serviceDateStr).getHours()]
            };

            // 이미 입력된 날짜인지 확인
            var filterSingleDate = scheduleList.singleDates.filter(function(
                    each) {
              return each.date === this.serviceDate;
            });
